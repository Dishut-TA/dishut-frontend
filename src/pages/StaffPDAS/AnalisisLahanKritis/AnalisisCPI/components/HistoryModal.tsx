...
import { getProjectsAPI } from '@/services/gisService';
...
const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, onSelectProject, activeProjectId }) => {
  ...
  const fetchHistory = async () => {
    setIsLoading(true);
    try {
      const json = await getProjectsAPI(50);
      const dataList = Array.isArray(json?.payload)
        ? json.payload
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-xl">
        <div className="p-4 md:p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Pilih data peta lama untuk ditampilkan. Setelah dipilih, data itu juga dapat diedit dan dianalisis ulang.</p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col gap-3">
              {projects.map((project) => {
                const isActive = Number(project.id) === Number(activeProjectId);
                return (
                  <div key={project.id} className={`border rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors ${isActive ? 'border-emerald-300 bg-emerald-50/40' : 'border-gray-200 hover:border-[#185325]/30 hover:bg-green-50/10'}`}>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-bold text-gray-800">{project.nama_project || project.kode_project || project.project_code}</h3>
                        {isActive && <span className="text-[10px] font-bold uppercase text-emerald-700 bg-white border border-emerald-200 px-2 py-0.5 rounded-full">Data aktif</span>}
                      </div>
                      <div className="text-sm text-gray-500 mt-1 flex flex-wrap items-center gap-3">
                        <span>{new Date(project.updated_at || project.created_at).toLocaleString('id-ID')}</span>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap flex items-center gap-2 ${
                        project.status === 'completed'
                          ? 'bg-[#185325] text-white hover:bg-[#113d1b] cursor-pointer'
                          : 'bg-gray-100 text-gray-400 cursor-not-allowed'
                      }`}
                    >
                      <HiOutlinePencilSquare className="w-4 h-4" />
                      {isActive ? 'Buka Data Aktif' : 'Buka & Edit'}
                    </button>
                  </div>
                );
              })}
